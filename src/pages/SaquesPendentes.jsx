...
export default function SaquesPendentes() {
  ...
  useEffect(() => {
    async function fetchSaques() {
      try {
        setLoading(true);
        setError(null);
        const result = await postData('/admin/saques/pendentes', {});
        setSaques(result.saques || []);
      } catch (error) {
        console.error('Erro ao buscar saques pendentes:', error);
        setError('Não foi possível carregar os saques pendentes. Tente novamente mais tarde.');
        setSaques([]);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="bg-background text-foreground min-h-screen p-6">
        <div className="bg-card p-8 rounded shadow-md max-w-4xl mx-auto">
          <h1 className="text-2xl font-bold text-yellow-600 mb-4">Saques Pendentes</h1>
  ...
  return (
    <div className="bg-background text-foreground min-h-screen p-6">
      <div className="bg-card p-8 rounded shadow-md max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold text-yellow-600 mb-4">Saques Pendentes</h1>
        {saques.length === 0 ? (
          <p className="text-gray-700 text-center p-8">Nenhum saque pendente no momento.</p>
        ) : (
