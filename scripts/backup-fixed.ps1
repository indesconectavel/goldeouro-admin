...
switch ($Action) {
    ...
    "restore" {
        if (-not $BackupName) {
            ...
            Write-ColorOutput "Uso: .\backup-fixed.ps1 restore BACKUP-2025-01-09T10-30-00" "Yellow"
            exit 1
        }
...
Exemplos:
  .\backup-fixed.ps1 create
  .\backup-fixed.ps1 quick
  .\backup-fixed.ps1 list
  .\backup-fixed.ps1 restore BACKUP-2025-01-09T10-30-00
