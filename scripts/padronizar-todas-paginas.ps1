# Script para padronizar todas as páginas responsivas com Design System
Write-Host "🚀 PADRONIZAÇÃO COMPLETA DE TODAS AS PÁGINAS" -ForegroundColor Green
...
foreach ($pagina in $paginas) {
    ...
    if (Test-Path $arquivo) {
        Write-Host "✅ $pagina - $arquivo" -ForegroundColor Green
    } else {
...
foreach ($pagina in $paginas) {
    ...
    if (Test-Path $arquivo) {
        try {
            Write-Host "`n🔧 Padronizando $pagina..." -ForegroundColor Yellow
            ...
            $template = New-StandardPageTemplate -PageName $pagina -Title $pagina -Description "Página de $($pagina) padronizada com Design System"
            
            # Salvar arquivo padronizado
            $template | Out-File -FilePath $arquivo -Encoding UTF8
            
            Write-Host "✅ $pagina padronizada com sucesso" -ForegroundColor Green
...
Write-Host "2. Acesse: http://localhost:5173/lista-usuarios" -ForegroundColor White
...
Write-Host "5. Ajuste o conteúdo específico de cada página" -ForegroundColor White
