...
const SidebarFixed = () => {
  ...
  const { device, windowSize, isDesktop, isTablet, isMobile } = useDeviceDetection();
  ...
  // Detectar Mobile Horizontal
  const isMobileHorizontal = isMobile && windowSize.width > windowSize.height;
  ...
  // Desktop: Sidebar fixa (comportamento original)
  if (isDesktop) {
    return (
      <aside className="w-64 bg-[#111827] text-white shadow-lg">
        {/* Header fixo */}
  ...
  // Mobile Horizontal: Sidebar OCULTA
  if (isMobileHorizontal) {
    return null;
  }

  // Mobile Vertical e Tablet: Sidebar retrátil
  return (
    <>
