    ...
    return (
      <div className="bg-[#000717] text-white min-h-screen p-4">
        <div className="max-w-6xl mx-auto">
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-yellow-400 mb-2">💾 Backup do Sistema</h1>
            <p className="text-gray-400 text-sm">Gerencie backups dos dados, configurações e logs</p>
          </div>
          ...
          {/* Ações de Backup */}
          <div className="bg-[#1A202C] rounded-lg p-4 border border-gray-700 mb-6">
            <h2 className="text-lg font-bold text-yellow-400 mb-4">🔄 Criar Novo Backup</h2>
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => handleCreateBackup('completo')}
                disabled={creating}
                className="p-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 text-white font-semibold rounded-lg transition duration-200"
              >
              ...
              <button
                ...
                disabled={creating}
                className="p-3 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white font-semibold rounded-lg transition duration-200"
              >
                <div className="text-xl mb-1">📊</div>
                <div className="text-xs">Dados</div>
              </button>

              <button
                onClick={() => handleCreateBackup('configuracoes')}
                disabled={creating}
                className="p-3 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 text-white font-semibold rounded-lg transition duration-200"
              >
              ...
              <button
                ...
                disabled={creating}
                className="p-3 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-600 text-white font-semibold rounded-lg transition duration-200"
              >
                <div className="text-xl mb-1">📝</div>
          ...
          {/* Lista de Backups */}
          <div className="bg-[#1A202C] rounded-lg p-4 border border-gray-700">
            <h2 className="text-lg font-bold text-yellow-400 mb-4">📋 Backups Disponíveis</h2>
                  ...
                  <div key={backup.id} className="bg-gray-800 rounded-lg p-4 border border-gray-600">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        <div className="text-xl">{getTypeIcon(backup.type)}</div>
                        <div className="flex-1">
                          <h3 className="text-sm font-semibold text-white truncate">{backup.name}</h3>
                          <p className="text-xs text-gray-400 mt-1">{backup.description}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <div>
                        ...
                        <p className={`text-xs px-2 py-1 rounded-full ${getStatusColor(backup.status)}`}>
                          {backup.status}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400">TAMANHO</p>
                        <p className="text-xs text-white">{backup.size}</p>
                      </div>
                    </div>

                    <div className="mb-3">
                      <p className="text-xs text-gray-400">CRIADO</p>
                      <p className="text-xs text-white">{new Date(backup.createdAt).toLocaleString('pt-BR')}</p>
                    </div>
                    
                    <div className="flex space-x-2">
                      <button
                        onClick={() => handleDownloadBackup(backup)}
                        className="flex-1 px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded transition duration-200"
                      >
                        📥 Download
                      </button>
                      ...
                      <button
                        ...
                        className="flex-1 px-2 py-1 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white text-xs rounded transition duration-200"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 px-2 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded transition duration-200"
                      >
                        🗑️
                      </button>
                    </div>
          ...
          <div className="grid grid-cols-1 gap-4 mt-6">
            <div className="bg-[#1A202C] rounded-lg p-4 border border-gray-700">
              <h3 className="text-lg font-bold text-yellow-400 mb-3">📊 Estatísticas</h3>
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Total de Backups:</span>
                  <span className="text-white font-semibold text-sm">{backups.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Backups Completos:</span>
                  <span className="text-white font-semibold text-sm">
                    {backups.filter(b => b.type === 'completo').length}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Último Backup:</span>
                  <span className="text-white font-semibold text-sm">
...
export default BackupResponsive;
